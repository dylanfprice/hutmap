'use strict';(function(){angular.module("hutmap",["hutmapServices","hutmapFilters","google-maps","ngResource","$strap.directives"]).config(["$routeProvider",function($routeProvider){}])})();(function(){angular.module("google-maps",[]).value("googleMapsDefaults",{"mapOptions":{zoom:8,center:new google.maps.LatLng(46.87916,-120),mapTypeId:google.maps.MapTypeId.ROADMAP}})})();(function(){angular.module("google-maps").directive("gmMarkers",["$log","$parse","$timeout","googleMapsUtils",function($log,$parse,$timeout,googleMapsUtils){var latLngEqual=googleMapsUtils.latLngEqual;var objToLatLng=googleMapsUtils.objToLatLng;function link(scope,element,attrs,controller){if(!("gmObjects"in attrs))throw"gmObjects attribute required";else if(!("gmGetLatLng"in attrs))throw"gmGetLatLng attribute required";var handlers={};angular.forEach(attrs,function(value,key){if(key.lastIndexOf("gmOn",
...
angular.forEach(orphaned,function(marker,i){var position=marker.getPosition();controller.removeMarker(position.lat(),position.lng())})};scope.$watch("gmObjects().length",function(newValue,oldValue){if(newValue!=null&&newValue!==oldValue)updateMarkers(scope.gmObjects())});$timeout(angular.bind(this,updateMarkers,scope.gmObjects()))}return{restrict:"AE",priority:100,scope:{gmObjects:"&",gmGetLatLng:"&",gmMarkerOptions:"&"},require:"^gmMap",link:link}}])})();(function(){angular.module("google-maps").factory("googleMapControllerFactory",["googleMapsUtils","googleMapsDefaults","googleMapsContainer",function(googleMapsUtils,googleMapsDefaults,googleMapsContainer){var latLngEqual=googleMapsUtils.latLngEqual;var boundsEqual=googleMapsUtils.boundsEqual;var hasNaN=googleMapsUtils.hasNaN;var gMDefaults=googleMapsDefaults;var gMContainer=googleMapsContainer;var MapController=function($scope,$element,$attrs){var mapId=$attrs.gmMapId;if(!mapId)throw"googleMap must have non-empty gmMapId attribute";
...
this._initDragListeners()};MapController.precision=3;MapController.prototype._getConfig=function($scope,gMDefaults){var defaults=gMDefaults.mapOptions;var config={};angular.extend(config,defaults,$scope.gmMapOptions());return config};MapController.prototype._createMap=function(id,element,config,gMContainer){var map=gMContainer.getMap(id);if(!map){map=new google.maps.Map(element[0],config);gMContainer.addMap(id,map)}else throw"A map with id "+id+" already exists. You must use"+" different ids for each instance of the googleMap directive.";
return map};MapController.prototype._initDragListeners=function(){var self=this;this.addMapListener("dragstart",function(){self.dragging=true});this.addMapListener("idle",function(){self.dragging=false});this.addMapListener("drag",function(){self.dragging=true})};MapController.prototype.addMapListener=function(event,handler){google.maps.event.addListener(this._map,event,handler)};MapController.prototype.addMapListenerOnce=function(event,handler){google.maps.event.addListenerOnce(this._map,event,handler)};
MapController.prototype.addListener=function(object,event,handler){google.maps.event.addListener(object,event,handler)};MapController.prototype.addListenerOnce=function(object,event,handler){google.maps.event.addListenerOnce(object,event,handler)};MapController.prototype.addMarker=function(markerOptions){var opts={};angular.extend(opts,markerOptions);if(!(opts.position instanceof google.maps.LatLng))throw"markerOptions did not contain a position";var marker=new google.maps.Marker(opts);var position=
marker.getPosition();if(this.hasMarker(position.lat(),position.lng()))return false;var hash=position.toUrlValue(this.precision);this._markers[hash]=marker;marker.setMap(this._map);return true};MapController.prototype.hasMarker=function(lat,lng){return this.getMarker(lat,lng)instanceof google.maps.Marker};MapController.prototype.getMarker=function(lat,lng){if(lat==null||lng==null)throw"lat or lng was null";var latLng=new google.maps.LatLng(lat,lng);var hash=latLng.toUrlValue(this.precision);if(hash in
this._markers)return this._markers[hash];else return null};MapController.prototype.removeMarker=function(lat,lng){if(lat==null||lng==null)throw"lat or lng was null";var latLng=new google.maps.LatLng(lat,lng);var removed=false;var hash=latLng.toUrlValue(this.precision);var marker=this._markers[hash];if(marker){marker.setMap(null);removed=true}this._markers[hash]=null;return removed};MapController.prototype.fitToMarkers=function(){var bounds=new google.maps.LatLngBounds;this.forEachMarker(function(marker){bounds.extend(marker.getPosition())});
this._map.fitBounds(bounds)};MapController.prototype.forEachMarker=function(fn){if(fn==null)throw"fn was null or undefined";angular.forEach(this._markers,function(marker,hash){fn(marker)})};return{MapController:MapController}}])})();(function(){angular.module("google-maps").directive("gmMap",["$timeout","googleMapControllerFactory",function($timeout,googleMapControllerFactory){function link(scope,element,attrs,controller){if(!angular.isDefined(scope.gmCenter))scope.center={};if(!angular.isDefined(scope.gmBounds))scope.bounds={};if(!attrs.hasOwnProperty("gmMapId"))throw"googleMap must have non-empty gmMapId attribute";var hasCenter=false;var hasZoom=false;var hasBounds=false;if(attrs.hasOwnProperty("gmCenter"))hasCenter=true;
...
"Found on a map and surveyed by the Hutmap team."]}]).controller("NavbarCtrl",["$scope","$window",function($scope,$window){$scope.isPath=function(path){return $window.location.pathname===path}}])})();(function(){angular.module("hutmap").value("version","0.1").constant("hutmapMapId","map_canvas").value("mapOptions",{zoom:3,center:new google.maps.LatLng(46.87916,-120),mapTypeId:google.maps.MapTypeId.ROADMAP,streetViewControl:false}).value("markerOptions",{huts:{icon:"/static/img/marker_gray_small.png",shape:{coord:[6,0,2,2,0,6,6,13,12,6,10,2,6,0],type:"poly"},zIndex:0},selected:{icon:"/static/img/marker_blue.png",zIndex:1}}).run(["hutmapMapId","googleMapsContainer",function(hutmapMapId,googleMapsContainer){var gmapPromise=
