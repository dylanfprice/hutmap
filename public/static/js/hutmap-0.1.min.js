...
["$scope",function($scope){$scope.accuracy_tooltip=["Coordinates provided, but unverifiable.","Wild ass guess.","Slightly better than a wild ass guess.","Found structure on satellite or topo map.","Surveyed with GPS by the Hutmap team.","Found on a map and surveyed by the Hutmap team."]}]).controller("SearchCtrl",["$scope","$location","$route","$log","Places",function($scope,$location,$route,$log,Places){$scope.submitting=false;$scope.lastQuery;$scope.selected;$scope.getPlaces=function(query){$scope.lastQuery=
query;if(query&&!$scope.submitting)return Places.getPlacePredictions(query);else return[]};var logError=function(status){$scope.submitting=false;$log.error(status)};var selectPlace=function(place){$scope.submitting=false;$route.reload();$location.path("/map/").search("m_center",place.geometry.location.toUrlValue()).search("m_zoom",8);if(place.geometry.viewport)$location.search("h_bbox",place.geometry.viewport.toUrlValue())};$scope.submit=function(){if($scope.selected){$scope.submitting=true;var selected=
$scope.selected;if(selected.reference)Places.getDetails(selected.reference).then(function(place){selectPlace(place)},logError)}else{var lastQuery=$scope.lastQuery;if(lastQuery){$scope.submitting=true;Places.getPlacePredictions(lastQuery).then(function(predictions){Places.getDetails(predictions[0].reference).then(function(place){selectPlace(place)},logError)},logError)}}}}])})();(function(){angular.module("hutmap").controller("MapCtrl",["$scope","$location","$timeout","$q","googleMapsContainer","hutmapMapId","mapOptions","markerOptions","utils",function($scope,$location,$timeout,$q,googleMapsContainer,hutmapMapId,mapOptions,markerOptions,utils){var gmapPromise=googleMapsContainer.getMapPromise(hutmapMapId);var scopeInitialized=$q.defer();var hutsInitialized=$q.defer();$scope.hutmapMapId=hutmapMapId;$scope.mapOptions=mapOptions;$scope.markerOptions=markerOptions;$scope.center;
