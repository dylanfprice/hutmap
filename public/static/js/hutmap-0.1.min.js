...
function(a,b){null!=a&&!isNaN(a)&&a!==b&&(e.zoom=a)});h&&b.$watch("gmBounds",function(a,b){a!==b&&!e.dragging&&a&&(e.bounds=a)})}}}])})();
(function(){angular.module("AngularGM").directive("gmMarkers",["$log","$parse","$timeout","angulargmUtils",function(a,b,d,c){c.latLngEqual;var e=c.objToLatLng;return{restrict:"AE",priority:100,scope:{gmObjects:"&",gmGetLatLng:"&",gmMarkerOptions:"&",gmEvent:"&"},require:"^gmMap",link:function(a,c,h,k){if(!("gmObjects"in h))throw"gmObjects attribute required";if(!("gmGetLatLng"in h))throw"gmGetLatLng attribute required";var n={};angular.forEach(h,function(a,c){if(0===c.lastIndexOf("gmOn",0)){var j=
angular.lowercase(c.substring(4)),e=b(a);n[j]=e}});var j=function(b){var c=a.gmMarkerOptions(),j={};angular.forEach(b,function(b){var l=a.gmGetLatLng({object:b}),f=e(l);if(null!=f){var h=f.toUrlValue(k.precision);if(j[h]=b,!k.hasMarker(l.lat,l.lng)){h={};angular.extend(h,c,{position:f});k.addMarker(h);var q=k.getMarker(l.lat,l.lng);angular.forEach(n,function(c,l){k.addListener(q,l,function(){d(function(){c(a.$parent.$parent,{object:b,marker:q})})})})}}});var f=[];k.forEachMarker(function(a){a.getPosition().toUrlValue(k.precision)in
j||f.push(a)});angular.forEach(f,function(a){a=a.getPosition();k.removeMarker(a.lat(),a.lng())})};a.$watch("gmObjects().length",function(b,c){null!=b&&b!==c&&j(a.gmObjects())});a.$watch("gmObjects()",function(b,c){null!=b&&b!==c&&j(a.gmObjects())});a.$watch("gmEvent()",function(a,b){if(null!=a&&a!==b){var c=a.event,j=a.location,j=k.getMarker(j.lat(),j.lng());null!=j&&d(angular.bind(this,k.trigger,j,c))}});d(angular.bind(null,j,a.gmObjects()))}}}])})();
(function(){angular.module("AngularGM").factory("angulargmContainer",["$q",function(a){var b={},d={};return{addMap:function(a,e){if(!(e instanceof google.maps.Map))throw"map not a google.maps.Map: "+e;if(a in b)throw"already contain map with id "+a;b[a]=e;a in d&&d[a].resolve(e)},getMap:function(a){return b[a]},getMapPromise:function(b){var e=d[b]||a.defer();return d[b]=e,e.promise},removeMap:function(a){a in b&&delete b[a];a in d&&delete d[a]},clear:function(){b={};d={}}}}])})();
...
angular.module("ui.bootstrap.carousel",["ui.bootstrap.transition"]).controller("CarouselController",["$scope","$timeout","$transition","$q",function(a,b,d){function c(){function e(){k?(a.next(),c()):a.pause()}h&&b.cancel(h);var j=+a.interval;!isNaN(j)&&0<=j&&(h=b(e,j))}var e=this,g=e.slides=[],f=-1,h,k;e.currentSlide=null;e.select=function(h,j){function l(){if(e.currentSlide&&angular.isString(j)&&!a.noTransition&&h.$element){h.$element.addClass(j);h.$element.attr("offsetWidth",h.$element[0].offsetWidth);
angular.forEach(g,function(a){angular.extend(a,{direction:"",entering:!1,leaving:!1,active:!1})});angular.extend(h,{direction:j,active:!0,entering:!0});angular.extend(e.currentSlide||{},{direction:j,leaving:!0});a.$currentTransition=d(h.$element,{});var b=h,l=e.currentSlide;a.$currentTransition.then(function(){m(b,l)},function(){m(b,l)})}else m(h,e.currentSlide);e.currentSlide=h;f=p;c()}function m(b,c){angular.extend(b,{direction:"",active:!0,leaving:!1,entering:!1});angular.extend(c||{},{direction:"",
active:!1,leaving:!1,entering:!1});a.$currentTransition=null}var p=g.indexOf(h);void 0===j&&(j=p>f?"next":"prev");h&&h!==e.currentSlide&&(a.$currentTransition?(a.$currentTransition.cancel(),b(l)):l())};e.indexOfSlide=function(a){return g.indexOf(a)};a.next=function(){return e.select(g[(f+1)%g.length],"next")};a.prev=function(){return e.select(g[0>f-1?g.length-1:f-1],"prev")};a.select=function(a){e.select(a)};a.isActive=function(a){return e.currentSlide===a};a.slides=function(){return g};a.$watch("interval",
c);a.play=function(){k||(k=!0,c())};a.pause=function(){k=!1;h&&b.cancel(h)};e.addSlide=function(b,c){b.$element=c;g.push(b);1===g.length||b.active?(e.select(g[g.length-1]),1==g.length&&a.play()):b.active=!1};e.removeSlide=function(a){var b=g.indexOf(a);g.splice(b,1);0<g.length&&a.active&&(b>=g.length?e.select(g[b-1]):e.select(g[b]))}}]).directive("carousel",[function(){return{restrict:"EA",transclude:!0,replace:!0,controller:"CarouselController",require:"carousel",templateUrl:"template/carousel/carousel.html",
...
var q=e.find("body");p.prototype.isOpen=function(){return this._open};p.prototype.open=function(a,b){var c=this,l=this.options;a&&(l.templateUrl=a);b&&(l.controller=b);if(!l.template&&!l.templateUrl)throw Error("Dialog.open expected template or templateUrl, neither found. Use options or open method to specify them.");this._loadResolves().then(function(a){var b=a.$scope=c.$scope=a.$scope?a.$scope:f.$new();c.modalEl.html(a.$template);c.options.controller&&(a=h(c.options.controller,a),c.modalEl.contents().data("ngControllerController",
a));g(c.modalEl)(b);c._addElementsToDom();q.addClass(c.options.dialogOpenClass);setTimeout(function(){c.options.dialogFade&&c.modalEl.addClass(c.options.triggerClass);c.options.backdropFade&&c.backdropEl.addClass(c.options.triggerClass)});c._bindEvents()});this.deferred=n.defer();return this.deferred.promise};p.prototype.close=function(a){function b(a){a.removeClass(l.options.triggerClass)}function c(){l._open&&l._onCloseComplete(a)}var l=this,e=this._getFadingElements();q.removeClass(l.options.dialogOpenClass);
if(0<e.length)for(var f=e.length-1;0<=f;f--)j(e[f],b).then(c);else this._onCloseComplete(a)};p.prototype._getFadingElements=function(){var a=[];this.options.dialogFade&&a.push(this.modalEl);this.options.backdropFade&&a.push(this.backdropEl);return a};p.prototype._bindEvents=function(){this.options.keyboard&&q.bind("keydown",this.handledEscapeKey);this.options.backdrop&&this.options.backdropClick&&this.backdropEl.bind("click",this.handleBackDropClick)};p.prototype._unbindEvents=function(){this.options.keyboard&&
...
(d--,0===d&&this.backdropEl.remove());this._open=!1};p.prototype._loadResolves=function(){var a=[],b=[],e,j=this;this.options.template?e=n.when(this.options.template):this.options.templateUrl&&(e=c.get(this.options.templateUrl,{cache:k}).then(function(a){return a.data}));angular.forEach(this.options.resolve||[],function(c,e){b.push(e);a.push(angular.isString(c)?l.get(c):l.invoke(c))});b.push("$template");a.push(e);return n.all(a).then(function(a){var c={};angular.forEach(a,function(a,l){c[b[l]]=a});
c.dialog=j;return c})};return{dialog:function(a){return new p(a)},messageBox:function(a,b,c){return new p({templateUrl:"template/dialog/message.html",controller:"MessageBoxController",resolve:{model:function(){return{title:a,message:b,buttons:c}}}})}}}]});
...
2);1>c&&(c=1);c+a-1>b.numPages&&(c-=c+a-1-b.numPages);for(var d=c,a=c+a;d<a;d++)c=e(d,d,b.isActive(d),!1),b.pages.push(c);f&&(d=e(b.currentPage-1,k,!1,b.noPrevious()),b.pages.unshift(d),d=e(b.currentPage+1,n,!1,b.noNext()),b.pages.push(d));g&&(d=e(1,h,!1,b.noPrevious()),b.pages.unshift(d),d=e(b.numPages,j,!1,b.noNext()),b.pages.push(d));b.currentPage>b.numPages&&b.selectPage(b.numPages)});b.noPrevious=function(){return 1===b.currentPage};b.noNext=function(){return b.currentPage===b.numPages};b.isActive=
...
angular.module("ui.bootstrap.progressbar",["ui.bootstrap.transition"]).constant("progressConfig",{animate:!0,autoType:!1,stackedTypes:["success","info","warning","danger"]}).controller("ProgressBarController",["$scope","$attrs","progressConfig",function(a,b,d){var c=angular.isDefined(b.animate)?a.$eval(b.animate):d.animate,e=angular.isDefined(b.autoType)?a.$eval(b.autoType):d.autoType,g=angular.isDefined(b.stackedTypes)?a.$eval("["+b.stackedTypes+"]"):d.stackedTypes;this.makeBar=function(a,b,d){var n=
angular.isObject(a)?a.value:a||0;b=angular.isObject(b)?b.value:b||0;a=angular.isObject(a)&&angular.isDefined(a.type)?a.type:e?g[d||0]:null;return{from:b,to:n,type:a,animate:c}};this.addBar=function(b){a.bars.push(b);a.totalPercent+=b.to};this.clearBars=function(){a.bars=[];a.totalPercent=0};this.clearBars()}]).directive("progress",function(){return{restrict:"EA",replace:!0,controller:"ProgressBarController",scope:{value:"=",onFull:"&",onEmpty:"&"},templateUrl:"template/progressbar/progress.html",
link:function(a,b,d,c){a.$watch("value",function(a,b){c.clearBars();if(angular.isArray(a))for(var d=0,h=a.length;d<h;d++)c.addBar(c.makeBar(a[d],b[d],d));else c.addBar(c.makeBar(a,b))},!0);a.$watch("totalPercent",function(b){if(100<=b)a.onFull();else if(0>=b)a.onEmpty()},!0)}}}).directive("progressbar",["$transition",function(a){return{restrict:"EA",replace:!0,scope:{width:"=",old:"=",type:"=",animate:"="},templateUrl:"template/progressbar/bar.html",link:function(b,d){b.$watch("width",function(c){b.animate?
...
angular.module("ui.bootstrap.tooltip",[]).provider("$tooltip",function(){var a={placement:"top",animation:!0},b={};this.options=function(a){angular.extend(b,a)};this.$get=["$window","$compile","$timeout","$parse",function(d,c,e,g){return function(f,h,k){var n=angular.extend({},a,b),j="<"+f+'-popup title="{{tt_title}}" content="{{tt_content}}" placement="{{tt_placement}}" animation="tt_animation()" is-open="tt_isOpen"></'+f+"-popup>";return{restrict:"EA",scope:!0,link:function(a,b,p){function q(){var c,
j,g,f,h,p;if(a.tt_content){s&&e.cancel(s);r.css({top:0,left:0,display:"block"});b.after(r);j=b;f=j[0].getBoundingClientRect();c=j.prop("offsetWidth");j=j.prop("offsetHeight");g=f.top+d.pageYOffset;f=f.left+d.pageXOffset;h=r.prop("offsetWidth");p=r.prop("offsetHeight");switch(a.tt_placement){case "right":c={top:g+j/2-p/2+"px",left:f+c+"px"};break;case "bottom":c={top:g+j+"px",left:f+c/2-h/2+"px"};break;case "left":c={top:g+j/2-p/2+"px",left:f-h+"px"};break;default:c={top:g-p+"px",left:f+c/2-h/2+"px"}}r.css(c);
a.tt_isOpen=!0}}function t(){a.tt_isOpen=!1;angular.isDefined(a.tt_animation)&&a.tt_animation()?s=e(function(){r.remove()},500):r.remove()}var r=c(j)(a),s;p.$observe(f,function(b){a.tt_content=b});p.$observe(f+"Title",function(b){a.tt_title=b});p.$observe(f+"Placement",function(b){a.tt_placement=angular.isDefined(b)?b:n.placement});p.$observe(f+"Animation",function(b){a.tt_animation=angular.isDefined(b)?g(b):function(){return n.animation}});a.tt_isOpen=!1;angular.isDefined(k)?(b.bind(h,function(){a.$apply(q)}),
...
angular.module("ui.bootstrap.transition",[]).factory("$transition",["$q","$timeout","$rootScope",function(a,b,d){function c(a){for(var b in a)if(void 0!==g.style[b])return a[b]}var e=function(c,g,k){k=k||{};var n=a.defer(),j=e[k.animation?"animationEndEventName":"transitionEndEventName"],l=function(){d.$apply(function(){c.unbind(j,l);n.resolve(c)})};j&&c.bind(j,l);b(function(){angular.isString(g)?c.addClass(g):angular.isFunction(g)?g(c):angular.isObject(g)&&c.css(g);j||n.resolve(c)});n.promise.cancel=
function(){j&&c.unbind(j,l);n.reject("Transition cancelled")};return n.promise},g=document.createElement("trans");e.transitionEndEventName=c({WebkitTransition:"webkitTransitionEnd",MozTransition:"transitionend",OTransition:"oTransitionEnd",msTransition:"MSTransitionEnd",transition:"transitionend"});e.animationEndEventName=c({WebkitTransition:"webkitAnimationEnd",MozTransition:"animationend",OTransition:"oAnimationEnd",msTransition:"MSAnimationEnd",transition:"animationend"});return e}]);
...
function(a,b,d,c){var e=[9,13,27,38,40];return{require:"ngModel",link:function(g,f,h,k){var n=k.$modelValue,j=g.$eval(h.typeaheadMinLength)||1,l=c.parse(h.typeahead),m=g.$new();g.$on("$destroy",function(){m.$destroy()});var p=function(){m.matches=[];m.activeIdx=-1};p();m.query=void 0;k.$parsers.push(function(a){p();if(n)return a;if(a&&a.length>=j){var c={$viewValue:a};b.when(l.source(m,c)).then(function(b){if(a===k.$viewValue)if(0<b.length){m.activeIdx=0;for(var j=m.matches.length=0;j<b.length;j++)c[l.itemName]=
b[j],m.matches.push({label:l.viewMapper(m,c),model:b[j]});m.query=a}else p()},p)}});k.$render=function(){var a={};a[l.itemName]=n;f.val(l.viewMapper(m,a)||k.$viewValue);n=void 0};m.select=function(a){var b={};b[l.itemName]=n=m.matches[a].model;k.$setViewValue(l.modelMapper(m,b));k.$render()};f.bind("keydown",function(a){0===m.matches.length||-1===e.indexOf(a.which)||(a.preventDefault(),40===a.which?(m.activeIdx=(m.activeIdx+1)%m.matches.length,m.$digest()):38===a.which?(m.activeIdx=(m.activeIdx?m.activeIdx:
m.matches.length)-1,m.$digest()):13===a.which||9===a.which?m.$apply(function(){m.select(m.activeIdx)}):27===a.which&&(a.stopPropagation(),m.matches=[],m.$digest()))});d.find("body").bind("click",function(){m.matches=[];m.$digest()});g=a("<typeahead-popup matches='matches' active='activeIdx' select='select(activeIdx)' query='query'></typeahead-popup>")(m);f.after(g)}}}]).directive("typeaheadPopup",function(){return{restrict:"E",scope:{matches:"=",query:"=",active:"=",select:"&"},replace:!0,templateUrl:"template/typeahead/typeahead.html",
...
angular.module("template/tooltip/tooltip-popup.html",[]).run(["$templateCache",function(a){a.put("template/tooltip/tooltip-popup.html",'<div class="tooltip {{placement}}" ng-class="{ in: isOpen(), fade: animation() }">  <div class="tooltip-arrow"></div>  <div class="tooltip-inner" ng-bind="content"></div></div>')}]);angular.module("template/typeahead/typeahead.html",[]).run(["$templateCache",function(a){a.put("template/typeahead/typeahead.html",'<div class="dropdown clearfix" ng-class="{open: isOpen()}">    <ul class="typeahead dropdown-menu">        <li ng-repeat="match in matches" ng-class="{active: isActive($index) }" ng-mouseenter="selectActive($index)">            <a tabindex="-1" ng-click="selectMatch($index)" ng-bind-html-unsafe="match.label | typeaheadHighlight:query"></a>        </li>    </ul></div>')}]);!function(a){var b=function(a,b){this.init("tooltip",a,b)};b.prototype={constructor:b,init:function(b,e,d){var f;this.type=b;this.$element=a(e);this.options=this.getOptions(d);this.enabled=!0;e=this.options.trigger.split(" ");for(d=e.length;d--;)if(f=e[d],"click"==f)this.$element.on("click."+this.type,this.options.selector,a.proxy(this.toggle,this));else"manual"!=f&&(b="hover"==f?"mouseenter":"focus",f="hover"==f?"mouseleave":"blur",this.$element.on(b+"."+this.type,this.options.selector,a.proxy(this.enter,
this)),this.$element.on(f+"."+this.type,this.options.selector,a.proxy(this.leave,this)));this.options.selector?this._options=a.extend({},this.options,{trigger:"manual",selector:""}):this.fixTitle()},getOptions:function(b){b=a.extend({},a.fn[this.type].defaults,this.$element.data(),b);b.delay&&"number"==typeof b.delay&&(b.delay={show:b.delay,hide:b.delay});return b},enter:function(b){var e=a.fn[this.type].defaults,d={},f;this._options&&a.each(this._options,function(a,b){e[a]!=b&&(d[a]=b)},this);f=
a(b.currentTarget)[this.type](d).data(this.type);if(!f.options.delay||!f.options.delay.show)return f.show();clearTimeout(this.timeout);f.hoverState="in";this.timeout=setTimeout(function(){"in"==f.hoverState&&f.show()},f.options.delay.show)},leave:function(b){var e=a(b.currentTarget)[this.type](this._options).data(this.type);this.timeout&&clearTimeout(this.timeout);if(!e.options.delay||!e.options.delay.hide)return e.hide();e.hoverState="out";this.timeout=setTimeout(function(){"out"==e.hoverState&&
e.hide()},e.options.delay.hide)},show:function(){var b,e,d,f,h;e=a.Event("show");if(this.hasContent()&&this.enabled&&(this.$element.trigger(e),!e.isDefaultPrevented())){b=this.tip();this.setContent();this.options.animation&&b.addClass("fade");f="function"==typeof this.options.placement?this.options.placement.call(this,b[0],this.$element[0]):this.options.placement;b.detach().css({top:0,left:0,display:"block"});this.options.container?b.appendTo(this.options.container):b.insertAfter(this.$element);e=
this.getPosition();d=b[0].offsetWidth;b=b[0].offsetHeight;switch(f){case "bottom":h={top:e.top+e.height,left:e.left+e.width/2-d/2};break;case "top":h={top:e.top-b,left:e.left+e.width/2-d/2};break;case "left":h={top:e.top+e.height/2-b/2,left:e.left-d};break;case "right":h={top:e.top+e.height/2-b/2,left:e.left+e.width}}this.applyPlacement(h,f);this.$element.trigger("shown")}},applyPlacement:function(a,b){var d=this.tip(),f=d[0].offsetWidth,h=d[0].offsetHeight,k,n,j;d.offset(a).addClass(b).addClass("in");
k=d[0].offsetWidth;n=d[0].offsetHeight;"top"==b&&n!=h&&(a.top=a.top+h-n,j=!0);"bottom"==b||"top"==b?(h=0,0>a.left&&(h=-2*a.left,a.left=0,d.offset(a),k=d[0].offsetWidth),this.replaceArrow(h-f+k,k,"left")):this.replaceArrow(n-h,n,"top");j&&d.offset(a)},replaceArrow:function(a,b,d){this.arrow().css(d,a?50*(1-a/b)+"%":"")},setContent:function(){var a=this.tip(),b=this.getTitle();a.find(".tooltip-inner")[this.options.html?"html":"text"](b);a.removeClass("fade in top bottom left right")},hide:function(){var b=
this.tip(),e=a.Event("hide");this.$element.trigger(e);if(!e.isDefaultPrevented()){b.removeClass("in");if(a.support.transition&&this.$tip.hasClass("fade")){var d=setTimeout(function(){b.off(a.support.transition.end).detach()},500);b.one(a.support.transition.end,function(){clearTimeout(d);b.detach()})}else b.detach();this.$element.trigger("hidden");return this}},fixTitle:function(){var a=this.$element;if(a.attr("title")||"string"!=typeof a.attr("data-original-title"))a.attr("data-original-title",a.attr("title")||
...
b.prjov_.setMap(null);this.dragZoom_=null}};google.maps.Map.prototype.keyDragZoomEnabled=function(){return null!==this.dragZoom_};google.maps.Map.prototype.getDragZoomObject=function(){return this.dragZoom_}})();(function(){angular.module("hutmap","hutmapServices hutmapFilters AngularGM ngResource $strap.directives ui.bootstrap".split(" "))})();(function(){angular.module("hutmapServices",[]).factory("Huts",["$http",function(a){return{query:function(b,d,c){a.get("/huts/api/v1/hut/",{params:b}).success(d).error(c)}}}]).factory("Places",["$q",function(a){function b(a,b,c){c==google.maps.places.PlacesServiceStatus.OK?a.resolve(b):a.reject(c)}var d=angular.element('<div id="google-attributions"></div>'),c=new google.maps.places.PlacesService(d[0]),e=new google.maps.places.AutocompleteService;return{getDetails:function(d){var e=a.defer();c.getDetails({reference:d},
angular.bind(this,b,e));return e.promise},textSearch:function(d){var e=a.defer();c.textSearch(d,angular.bind(this,b,e));return e.promise},getPlacePredictions:function(c){var d=a.defer();e.getPlacePredictions({input:c},angular.bind(this,b,d));return d.promise}}}]).factory("utils",[function(){return{latLngFromUrlValue:function(a){a=a.split(",");return new google.maps.LatLng(a[0],a[1])},boundsFromUrlValue:function(a){a=a.split(",");return new google.maps.LatLngBounds(new google.maps.LatLng(a[0],a[1]),
new google.maps.LatLng(a[2],a[3]))}}}])})();(function(){angular.module("hutmap").config(["$routeProvider","$locationProvider",function(a,b){a.when("/",{templateUrl:"/partials/home.html",activetab:"home"}).when("/map/",{templateUrl:"/partials/map.html",activetab:"map",reloadOnSearch:!1}).when("/about/",{templateUrl:"/partials/about.html",activetab:"about"});b.html5Mode(!0)}]).constant("hutmapMapId","map_canvas").value("mapOptions",{zoom:8,center:new google.maps.LatLng(46.87916,-120),mapTypeId:google.maps.MapTypeId.ROADMAP,streetViewControl:!1}).value("markerOptions",
{huts:{icon:"/static/img/marker_gray_small.png",shape:{coord:[6,0,2,2,0,6,6,13,12,6,10,2,6,0],type:"poly"},zIndex:0},selected:{icon:"/static/img/marker_yellow_small.png",zIndex:1}})})();(function(){angular.module("hutmapFilters",[]).filter("truncate",function(){return function(a,b,d){isNaN(b)&&(b=10);void 0===d&&(d="...");return null==a||0==a.length?null:a.length<=b||a.length-d.length<=b?a:String(a).substring(0,b-d.length)+d}})})();(function(){angular.module("hutmap.directives",[])})();(function(){angular.module("hutmap").controller("HutCtrl",["$scope","$location","$timeout","Huts",function(a,b,d,c){var e=0;a.loading=0;a.huts;a.hutsMeta;a.query;a.selectedHut;a.incLoading=function(){a.loading++};a.decLoading=function(){a.loading--};a.setQuery=function(b){a.query=b};a.setSelectedHut=function(b){a.selectedHut=b};a.$watch("query",function(b){if(null!=b){var f=++e;d(function(){f===e&&b&&(a.incLoading(),c.query(b,function(b){a.decLoading();f===e&&(a.huts=b.objects,a.hutsMeta=b.meta)},
function(){a.decLoading()}))},500)}});a.$watch("hutsMeta.total_count",function(a){a&&console.log(a)})}])})();(function(){angular.module("hutmap").controller("HutmapCtrl",["$scope","$route",function(a,b){a.$route=b}]).controller("CarouselCtrl",["$scope","$route",function(a){a.carouselInterval=6E3;a.slides=[{title:"Big Hut",text:"lorem ipsum dolor",image:"/static/img/carousel/Big Hut.JPG"},{title:"Joe River Chickee",text:"lorem ipsum dolor",image:"/static/img/carousel/Joe River Chickee.JPG"},{title:"John Muir Shelter",text:"lorem ipsum dolor",image:"/static/img/carousel/John Muir Shelter.JPG"}]}]).controller("AlertCtrl",
["$scope",function(a){a.alerts=[];a.addAlert=function(b,d){a.alerts.push({type:b,msg:d})};a.closeAlert=function(b){a.alerts.splice(b,1)}}]).controller("HutInfoCtrl",["$scope",function(a){a.accuracy_tooltip="Coordinates provided, but unverifiable.;Wild ass guess.;Slightly better than a wild ass guess.;Found structure on satellite or topo map.;Surveyed with GPS by the Hutmap team.;Found on a map and surveyed by the Hutmap team.".split(";")}])})();(function(){angular.module("hutmap").controller("SearchCtrl",["$scope","$location","$route","$log","Places",function(a,b,d,c,e){a.submitting=!1;a.autocompleting=0;a.lastQuery;a.selected;a.getPlaces=function(b){return(a.lastQuery=b)&&!a.submitting?(a.autocompleting++,e.getPlacePredictions(b).then(function(b){a.autocompleting--;return a.submitting?[]:b},function(){a.autocompleting--;return[]})):[]};var g=function(b){a.submitting=!1;c.error(b);a.addAlert("error","There was an error retrieving search results. TODO: better messages")},
f=function(c){a.submitting=!1;d.reload();b.path("/map/").search({}).search("m_zoom",8);c.geometry.location&&(b.search("m_center",c.geometry.location.toUrlValue()),b.search("m_selected",c.geometry.location.toUrlValue()));c.geometry.viewport&&b.search("m_bounds",c.geometry.viewport.toUrlValue())};a.$watch("selected",function(b){b&&a.submit()});a.submit=function(){if(a.selected){a.submitting=!0;a.autocompleting=0;var b=a.selected;b.reference&&e.getDetails(b.reference).then(function(a){f(a)},g)}else if(b=
a.lastQuery)a.submitting=!0,a.autocompleting=0,e.getPlacePredictions(b).then(function(a){e.getDetails(a[0].reference).then(function(a){f(a)},g)},g)}}])})();(function(){angular.module("hutmap").controller("MapConfigCtrl",["$scope","hutmapMapId","angulargmContainer","mapOptions","markerOptions",function(a,b,d,c,e){d=d.getMapPromise(b);var g;a.hutmapMapId=b;a.mapOptions=c;a.markerOptions=e;a.selectHut=function(b,c){g&&g.setOptions(e.huts);g=b;b.setOptions(e.selected);a.setSelectedHut(c)};d.then(function(a){var b={MSR_TOPO:new google.maps.ImageMapType({name:"MSR USGS",alt:"USGS topos from Microsoft Research",minZoom:1,maxZoom:15,tileSize:new google.maps.Size(256,
256),isPng:!1,getTileUrl:function(b,c){var d=a.getProjection(),e=Math.pow(2,c),g=new google.maps.Point(256*b.x/e,256*(b.y+1)/e),e=new google.maps.Point(256*(b.x+1)/e,256*b.y/e),g=d.fromPointToLatLng(g),d=d.fromPointToLatLng(e);return"http://msrmaps.com/ogcmap.ashx?version=1.1.1&request=GetMap&Layers=drg&Styles=default&SRS=EPSG:4326&BBOX="+[g.lng(),g.lat(),d.lng(),d.lat()].join()+"&width=256&height=256&format=image/jpeg"}}),ARC_GIS_USA:new google.maps.ImageMapType({name:"ArcGIS USA",alt:"ArcGIS USA Topos",
minZoom:1,maxZoom:15,tileSize:new google.maps.Size(256,256),isPng:!0,getTileUrl:function(a,b){var c=1<<b,d=a.x%c;0>d&&(d+=c);return"http://services.arcgisonline.com/ArcGIS/rest/services/USA_Topo_Maps/MapServer/tile/"+parseInt(b)+"/"+a.y+"/"+d}}),ARC_GIS_WORLD:new google.maps.ImageMapType({name:"ArcGIS World",alt:"ArcGIS World Topos",minZoom:1,maxZoom:15,tileSize:new google.maps.Size(256,256),isPng:!0,getTileUrl:function(a,b){var c=1<<b,d=a.x%c;0>d&&(d+=c);return"http://services.arcgisonline.com/ArcGIS/rest/services/World_Topo_Map/MapServer/tile/"+
parseInt(b)+"/"+a.y+"/"+d}})};angular.forEach(b,function(b){a.mapTypes.set(b.name,b)});a.setOptions({mapTypeControlOptions:{mapTypeIds:[google.maps.MapTypeId.ROADMAP,google.maps.MapTypeId.SATELLITE,google.maps.MapTypeId.HYBRID,google.maps.MapTypeId.TERRAIN,b.MSR_TOPO.name,b.ARC_GIS_USA.name,b.ARC_GIS_WORLD.name]}});a.enableKeyDragZoom({key:"alt",visualEnabled:!0})})}])})();(function(){angular.module("hutmap").controller("MapCtrl",["$scope","$location","$timeout","$q","utils",function(a,b,d,c,e){var g=c.defer(),f=c.defer();a.center;a.zoom;a.bounds;a.hutMarkerEvent;var h=function(){f.promise.then(function(){a.setSelectedHut(null);var c=b.search().m_selected;null!=c&&(a.hutMarkerEvent={event:"click",location:e.latLngFromUrlValue(c)})})};d=function(c,d){if(c!==d&&(a.center&&b.search("m_center",a.center.toUrlValue()),a.zoom&&b.search("m_zoom",a.zoom),a.selectedHut)){var e=
a.selectedHut.location;b.search("m_selected",e.lat+","+e.lng)}};a.$watch("center != null && zoom != null",function(a){a&&g.resolve()});a.$watch("huts != null",function(a){a&&f.resolve()});a.$watch("center",d);a.$watch("zoom",d);a.$watch("selectedHut",d);a.$watch("bounds",function(b){b&&a.setQuery({bbox:b.toUrlValue(),limit:0})});a.$watch("huts",function(a){a&&h()});var k=b.search().m_center,n=b.search().m_zoom,j=b.search().m_bounds;b.search("m_center",null);b.search("m_zoom",null);b.search("m_bounds",
null);g.promise.then(function(){var b=null!=j;!b&&null!=k&&(a.center=e.latLngFromUrlValue(k));!b&&null!=n&&(a.zoom=Number(n));b&&(a.bounds=e.boundsFromUrlValue(j))});h()}])})();
