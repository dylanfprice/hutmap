...
angular.lowercase(j.substring(4)),c=b(a);l[k]=c}});var j=function(b){var j={};angular.forEach(b,function(b){var c=a.gmGetLatLng({object:b}),n=d(c);if(null!=n){var p=a.gmGetMarkerOptions({object:b}),f=n.toUrlValue(k.precision);if(j[f]=b,!k.hasMarker(c.lat,c.lng)){f={};angular.extend(f,p,{position:n});k.addMarker(f);var h=k.getMarker(c.lat,c.lng);angular.forEach(l,function(j,c){k.addListener(h,c,function(){e(function(){j(a.$parent.$parent,{object:b,marker:h})})})})}}});var c=[];k.forEachMarker(function(a){a.getPosition().toUrlValue(k.precision)in
...
angular.lowercase(j.substring(4)),d=b(a);l[c]=d}});var j=function(b){var j={};angular.forEach(b,function(b){var c=a.gmGetLatLng({object:b}),n=d(c);if(null!=n){var f=a.gmGetMarkerOptions({object:b}),p=n.toUrlValue(k.precision);if(j[p]=b,!k.hasMarker(c.lat,c.lng)){p={};angular.extend(p,f,{position:n});k.addMarker(p);var h=k.getMarker(c.lat,c.lng);angular.forEach(l,function(j,c){k.addListener(h,c,function(){e(function(){j(a.$parent.$parent,{object:b,marker:h})})})})}}});var c=[];k.forEachMarker(function(a){a.getPosition().toUrlValue(k.precision)in
...
angular.module("ui.bootstrap.transition",[]).factory("$transition",["$q","$timeout","$rootScope",function(a,b,e){function c(a){for(var b in a)if(void 0!==g.style[b])return a[b]}var d=function(c,f,g){g=g||{};var l=a.defer(),j=d[g.animation?"animationEndEventName":"transitionEndEventName"],n=function(){e.$apply(function(){c.unbind(j,n);l.resolve(c)})};j&&c.bind(j,n);b(function(){angular.isString(f)?c.addClass(f):angular.isFunction(f)?f(c):angular.isObject(f)&&c.css(f);j||l.resolve(c)});l.promise.cancel=
function(){j&&c.unbind(j,n);l.reject("Transition cancelled")};return l.promise},g=document.createElement("trans");d.transitionEndEventName=c({WebkitTransition:"webkitTransitionEnd",MozTransition:"transitionend",OTransition:"oTransitionEnd",transition:"transitionend"});d.animationEndEventName=c({WebkitTransition:"webkitAnimationEnd",MozTransition:"animationend",OTransition:"oAnimationEnd",transition:"animationend"});return d}]);
...
function(){this.modalEl.remove();this.options.backdrop&&(e--,0===e&&this.backdropEl.remove());this._open=!1};p.prototype._loadResolves=function(){var a=[],b=[],d,e=this;this.options.template?d=l.when(this.options.template):this.options.templateUrl&&(d=c.get(this.options.templateUrl,{cache:k}).then(function(a){return a.data}));angular.forEach(this.options.resolve||[],function(d,c){b.push(c);a.push(angular.isString(d)?n.get(d):n.invoke(d))});b.push("$template");a.push(d);return l.all(a).then(function(a){var d=
...
angular.isDefined(b)?b:n.placement});q.$observe(k+"Animation",function(b){a.tt_animation=angular.isDefined(b)?g(b):function(){return n.animation}});q.$observe(k+"PopupDelay",function(b){b=parseInt(b,10);a.tt_popupDelay=!isNaN(b)?b:n.popupDelay});a.tt_isOpen=!1;angular.isDefined(j)?(b.bind(l,function(){w()}),b.bind(j,function(){a.$apply(y)})):b.bind(l,function(){a.tt_isOpen?a.$apply(y):w()})}}}}]}).directive("tooltipPopup",function(){return{restrict:"E",replace:!0,scope:{content:"@",placement:"@",
...
function(a,b,e,c,d){var g=[9,13,27,38,40];return{require:"ngModel",link:function(h,f,k,l){var j,n=h.$eval(k.typeaheadMinLength)||1,q=d.parse(k.typeahead),p=!1!==h.$eval(k.typeaheadEditable),r=b(k.typeaheadLoading).assign||angular.noop,m=h.$new();h.$on("$destroy",function(){m.$destroy()});var t=function(){m.matches=[];m.activeIdx=-1};t();m.query=void 0;l.$parsers.push(function(a){t();if(j)return a;if(a&&a.length>=n){var b={$viewValue:a};r(h,!0);e.when(q.source(m,b)).then(function(c){if(a===l.$viewValue){if(0<
c.length){m.activeIdx=0;for(var d=m.matches.length=0;d<c.length;d++)b[q.itemName]=c[d],m.matches.push({label:q.viewMapper(m,b),model:c[d]});m.query=a}else t();r(h,!1)}},function(){t();r(h,!1)})}return p?a:void 0});l.$render=function(){var a={};a[q.itemName]=j||l.$viewValue;f.val(q.viewMapper(m,a)||l.$viewValue);j=void 0};m.select=function(a){var b={};b[q.itemName]=j=m.matches[a].model;l.$setViewValue(q.modelMapper(m,b));l.$render()};f.bind("keydown",function(a){0===m.matches.length||-1===g.indexOf(a.which)||
...
function(a){d.push(e.get(a.image))});c.all(d).then(function(){a.carouselInterval=8E3})}]).controller("AlertCtrl",["$scope",function(a){a.alerts=[];a.addAlert=function(b,e){a.alerts.push({type:b,msg:e})};a.closeAlert=function(b){a.alerts.splice(b,1)}}]).controller("HutInfoCtrl",["$scope",function(a){a.accuracy_tooltip="Coordinates provided, but unverifiable.;Wild ass guess.;Slightly better than a wild ass guess.;Found structure on satellite or topo map.;Surveyed with GPS by the Hutmap team.;Found on a map and surveyed by the Hutmap team.".split(";")}])})();(function(){angular.module("hutmap").controller("SearchCtrl",["$scope","$location","$route","$log","$q","$document","Places",function(a,b,e,c,d,g,h){a.submitting=!1;a.autocompleting=0;a.lastQuery;a.selected;a.clickBody=function(){g.find("body").triggerHandler("click")};a.blurInput=function(){g.find("input").trigger("blur")};a.getPlaces=function(b){a.lastQuery=b;var c=d.defer();a.autocompleting++;h.getPlacePredictions(b,function(b){a.$apply(function(){a.autocompleting--;c.resolve(b)})},function(){a.$apply(function(){a.autocompleting--;
c.resolve([])})});return c.promise};var f=function(b){a.submitting=!1;c.error(b);a.addAlert("error","There was an error retrieving search results. TODO: better messages")};a.$watch("selected",function(b){b&&b.constructor===Object&&a.submit(b)});a.submit=function(c){c&&(a.submitting=!0,a.autocompleting=0,c.reference&&h.getDetails(c.reference,function(c){a.$apply(function(){a.submitting=!1;e.reload();b.path("/map/").search({}).search("m_zoom",8);c.geometry.location&&(b.search("m_center",c.geometry.location.toUrlValue()),
