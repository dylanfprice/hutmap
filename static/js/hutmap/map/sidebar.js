...
hutmap.map.Sidebar.prototype.setHut = function(hut) {
  ...
  textContent = this.getFeeRange(hut, 'fee_person_min', 'fee_person_max');
  elt = goog.dom.getElement('sidebar_fee_person');
  goog.dom.setTextContent(elt, textContent);
  // Set the fee per hut
  textContent = this.getFeeRange(hut, 'fee_hut_min', 'fee_hut_max');
  ...
  textContent = goog.object.get(hut, 'capacity_max', 'Unknown');
  elt = goog.dom.getElement('sidebar_total_capacity');
  goog.dom.setTextContent(elt, textContent);
  // Set the number of structures
  textContent = goog.object.get(hut, 'num_structures', 'Unknown');
  elt = goog.dom.getElement('sidebar_structures');
  goog.dom.setTextContent(elt, textContent);
  // Set whether reservations are accepted
  textContent = this.getBooleanAsYesNo(hut, 'reservations');
  elt = goog.dom.getElement('sidebar_reservations');
  goog.dom.setTextContent(elt, textContent);
  // Set the region
  var region = goog.object.get(hut, 'region', null);
  if (region == null) {
    textContent = 'Unknown';
  } else {
  ...
  goog.dom.setTextContent(elt, textContent);
  // Set the location
  var location = goog.object.get(hut, 'location', null);
  if (location == null) {
    textContent = 'Unknown';
  } else {
  ...
  goog.dom.setTextContent(elt, textContent);
  // Set the accuracy
  var locationAccuracy = goog.object.get(hut, 'location_accuracy', null);
  ...
  goog.dom.setTextContent(elt, textContent);
  // Set the altitude
  var altitude = goog.object.get(hut, 'altitude', null);
  if (altitude == null) {
    textContent = 'Unknown';
  } else {
  ...
  goog.dom.setTextContent(elt, textContent);
  // Set the access
  var access = goog.object.get(hut, 'access', null);
  if (access == null) {
    textContent = 'Unknown';
  } else {
  ...
  goog.dom.setTextContent(elt, textContent);
  // Set the agency
  var agency = goog.object.get(hut, 'agency', null);
  if (agency == null) {
    textContent = 'Unknown';
  } else {
...
hutmap.map.Sidebar.prototype.getCapacityRange = function(hut) {
  var capMin = hut['capacity_hut_min'];
  var capMax = hut['capacity_hut_max'];
